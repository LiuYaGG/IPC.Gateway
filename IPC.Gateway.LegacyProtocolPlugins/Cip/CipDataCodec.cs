...
namespace IPC.Plc.Communication.Cip
{
    ...
    public static class CipDataCodec
    {
        public static object Decode(PlcDataType requestedType, ushort actualType, byte[] data, int count)
        {
            ...
            switch (requestedType)
            {
                ...
                case PlcDataType.String:
                    return actualType == CipTypeCodes.String
                        ? DecodeStandardCipString(data)
                        : DecodeRockwellString(data);
        ...
        private static string DecodeStandardCipString(byte[] data)
        {
            Require(data, 2);
            int length = BitConverter.ToUInt16(data, 0);
            if (length > data.Length - 2)
                throw new InvalidOperationException("标准 CIP STRING 返回的长度字段超过了有效数据长度。");
            return Encoding.Latin1.GetString(data, 2, length);
        }

        private static string DecodeRockwellString(byte[] data)
        {
            Require(data, 4);
        ...
        public static byte[] EncodeStandardCipString(string text)
        {
            byte[] bytes = Encoding.Latin1.GetBytes(text ?? string.Empty);
            if (bytes.Length > ushort.MaxValue)
                throw new ArgumentException("标准 CIP STRING 最大长度为 65535 字节。");

            int paddedLength = (bytes.Length & 1) == 0 ? bytes.Length : bytes.Length + 1;
            byte[] result = new byte[2 + paddedLength];
            Buffer.BlockCopy(BitConverter.GetBytes((ushort)bytes.Length), 0, result, 0, 2);
