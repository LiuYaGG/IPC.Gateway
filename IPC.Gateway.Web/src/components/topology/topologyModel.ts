...
function addServiceNodes(nodes: TopologyNode[], links: TopologyLink[], status: GatewayStatus | null) {
  const services = [
    ...
    { id: 'service:rules', name: '规则引擎', tone: moduleTone(status?.flowRuleEngine?.enabled, status?.flowRuleEngine?.isRunning, status?.flowRuleEngine?.lastError), message: status?.flowRuleEngine?.lastError || '-' }
  ]
