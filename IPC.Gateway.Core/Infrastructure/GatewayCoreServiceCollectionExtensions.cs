...
public static class GatewayCoreServiceCollectionExtensions
{
    public static IServiceCollection AddGatewayCore(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        ConfigureProtocolDriverTrust(configuration);
    ...
    private static GatewayRuntimeOptions CreateRuntimeOptions(IConfiguration configuration, GatewaySecretProtectionOptions secretProtectionOptions)
    {
        ...
        return new GatewayRuntimeOptions
        {
            ...
            Scheduler = new RuntimeSchedulerOptions
            {
                ...
                DeviceStatusFailureDebounceCount = GetInt(section, "DeviceStatusFailureDebounceCount", 2),
                DeviceStatusFailureDebounceMs = GetInt(section, "DeviceStatusFailureDebounceMs", 2000),
                DeviceStatusRecoveryDebounceCount = GetInt(section, "DeviceStatusRecoveryDebounceCount", 1),
                DeviceStatusRecoveryDebounceMs = GetInt(section, "DeviceStatusRecoveryDebounceMs", 0),
                TagValueChangedQueueLimit = GetInt(section, "TagValueChangedQueueLimit", 100000)
            }
        };
    }

    private static void ConfigureProtocolDriverTrust(IConfiguration configuration)
    {
        IConfigurationSection section = configuration.GetSection("Gateway:ProtocolDrivers");
        PlcDriverPluginRegistry.ConfigureTrustPolicy(
            GetBool(section, "RequireSignature", false),
            section["TrustedPublicKeyPem"] ?? string.Empty);
    }
    ...
    private static LocalHistoryOptions CreateHistoryOptions(IConfiguration configuration)
    {
        ...
        return new LocalHistoryOptions
        {
            ...
            BatchWriteEnabled = GetBool(section, "BatchWriteEnabled", true),
            BatchFlushIntervalMs = GetInt(section, "BatchFlushIntervalMs", 1000),
            BatchMaxLines = GetInt(section, "BatchMaxLines", 1000),
            BatchQueueLimit = GetInt(section, "BatchQueueLimit", 100000),
