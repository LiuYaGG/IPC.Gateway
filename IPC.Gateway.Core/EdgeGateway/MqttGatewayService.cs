...
namespace IPC.EdgeGateway
{
    ...
    public sealed class MqttGatewayService : IDisposable
    {
        ...
        private void OnRuntimeTagValueChanged(object? sender, TagValueChangedEventArgs e)
        {
            ...
            TryQueueAlarmPublish(e.Snapshot);

            if (!_options.PublishEnabled)
                return;
            if (_options.PublishSelectedTagsOnly && !e.Snapshot.MqttPublishEnabled)
                return;
            ...
            try
            {
                ...
                if (!ShouldQueuePublish(e.Snapshot, publishUtc, out stateKey, out stateSignature))
                    return;

                string topic;
                if (IsSparkplugMode())
                {
                    byte[] payload = BuildSparkplugDataPayload(e.Snapshot);
                    topic = BuildSparkplugDeviceDataTopic(e.Snapshot);
                    _outboxStore.Enqueue(topic, payload, _options.PublishQos);
                    RecordPublish("sparkplugData", topic, _options.PublishQos, payload);
                }
                else
                {
                    string payload = BuildPublishPayload(e.Snapshot);
                    topic = BuildPublishTopic(e.Snapshot);
                    _outboxStore.Enqueue(topic, payload, _options.PublishQos);
